/* Modified by Sergio E. Garcia Tapia for Exercise 63-8 */
...
/* Exercise 63-8 (based on Listing 63-3) */

/* demo_sigio.c

   A trivial example of the use of signal-driven I/O.
*/
#define _GNU_SOURCE                 /* Expose F_SETSIG */
#include <signal.h>
#include <ctype.h>
#include <unistd.h>
#include <fcntl.h>
...
sigioHandler(int sig, siginfo_t *si, void *ucontext)
{
    ...
    printf("si_fd=%d, si_code=%d (%s)\n", si->si_fd, si->si_code, si->si_code == POLL_IN ? "POLL_IN" : "Not POLL_IN");
}
...
main(int argc, char *argv[])
{
    ...
    if (sigaction(SIGRTMIN + RTSIG_OFFSET, &sa, NULL) == -1)
        errExit("sigaction");
    ...
    /* Change default signal for signal I/O */
    if (fcntl(STDIN_FILENO, F_SETSIG, SIGRTMIN + RTSIG_OFFSET) == -1)
