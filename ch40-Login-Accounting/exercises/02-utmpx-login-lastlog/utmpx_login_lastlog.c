/* Modified by Sergio E. Garcia Tapia for Exercise 40.2 */
...
#include <sys/utsname.h> /* uname() to get hostname */
#include "tlpi_hdr.h"

static void
updateLastlog(char *username, char *terminalName)
{
	int fd;
	struct passwd *pw;
	struct lastlog llog;
	...
	strncpy(llog.ll_line, terminalName, sizeof(llog.ll_line));

	/* Get hostname */
	if (uname(&uts) == -1)
		errExit("uname");
	strncpy(llog.ll_host,  uts.nodename, sizeof(llog.ll_host));
	...
	/* Index to correct location in file */
	if (lseek(fd, pw->pw_uid * sizeof(struct lastlog), SEEK_SET) == -1)
		errExit("lseek");

	/* Update lastlog */
	printf("Creating lastlog entry with\n"
				"\tusing user %s (%ld), terminal %s, and hostname %s\n",
				username, (long) pw->pw_uid, terminalName, uts.nodename);
